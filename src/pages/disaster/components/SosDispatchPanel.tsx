...
export default function SosDispatchPanel({ sos, handleCallPhone }: SosDispatchPanelProps) {
  ...
  const dispatchStatusLabel = sos.status === 'DISPATCHED' || sos.status === 'ON_SITE'
    ? statusLabels[sos.status as keyof typeof statusLabels]
    : hasTeam
      ? 'Đã phân công'
      : 'Đang tìm đội phù hợp';

  const dispatchStatusColor = hasTeam ? 'text-blue-600 dark:text-blue-400' : 'text-amber-600 dark:text-amber-400';
